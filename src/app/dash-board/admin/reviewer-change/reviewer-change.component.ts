...
@Component({
  selector: 'app-reviewer-change',
  templateUrl: './reviewer-change.component.html',
  styleUrls: ['./reviewer-change.component.css']
})
export class ReviewerChangeComponent implements OnInit {
  TaskAllocationForm: FormGroup | any;
  submitted = false;
  public gridOptions: GridOptions | any;
  onFirstDataRendered: any;
  frameworkComponents: any;
  changeReviewerName?: string | any;
  ...
  changeReviewerAllocations: Taskallocation001wb[] = [];
  curators: User001mb[] = [];
  reviewers: User001mb[] = [];
  users: User001mb[] = [];
  username: any;
  reviewernames: User001mb[] = [];
  ...
  loadData() {
    ...
    this.userManager.allReviewerName().subscribe((response) => {
      this.reviewernames = deserialize<User001mb[]>(User001mb, response);
  ...
  onGetSelectedRowData(changeReviewerName: any) {
    ...
    for (let i = 0; i < this.selectedData.length; i++) {
      ...
      taskallocation001wb.curatorName = this.selectedData[i].curatorName;
      ...
      taskallocation001wb.reviewerName = changeReviewerName;
    ...
    this.calloutService.showSuccess("Reviewer Name Changed Successfully");
  }

  onSearch(rbatchNo: any, reviewerName: any) {
   
    this.changeReviewerAllocations = [];
    for (let i = 0; i < this.taskallocations.length; i++) {

      if ((this.taskallocations[i].rbatchNo == rbatchNo) && (this.taskallocations[i].reviewerName == reviewerName)) {
        this.changeReviewerAllocations.push(this.taskallocations[i]);

      }
    }
    this.rbatchNo="";
    this.reviewerName="";
    if (this.changeReviewerAllocations.length > 0) {
      this.gridOptions?.api?.setRowData(this.changeReviewerAllocations);
    } else {
      this.gridOptions?.api?.setRowData([]);
    }
